...
class AgentRule():
    def __init__(self, config):
        ...
        def build_facet_index_dict():
            ...
            for facet, dim_num in zip(self.all_facet_list, self.facet_dim_list):
                facet_index_dict[facet] = (start_idx, start_idx + dim_num) # 确定fact对应的index维度
                start_idx += dim_num
    ...
    def next_turn(self, dialogue_state):
        ...
        for facet in self.all_facet_list:
            facet_entropy = torch.tensor(0.).to(self.device)
            # if self.use_gpu:
            #     facet_entropy = torch.tensor(0.).cuda()
        ...
        # if self.use_gpu:
        facet_entropy_tensor = torch.stack(facet_entropy_list).to(self.device)
        # else:
