...
class PretrainBatchData():
    def __init__(self, state_list, label_list, device,output_check=False):
        batch_size = len(state_list)
        self.device = device
        self.state_list = torch.tensor(state_list).to(self.device)
        self.label_list = torch.tensor(label_list).to(self.device)
        # if use_gpu:
...
class AgentRL():
    def __init__(self, config):
        ...
        self.DPN = TwoLayersModel(config).to(self.device)
    ...
    def load_pretrain_data(self):
        ...
        def make_batch_data(state_list, label_list, batch_size):
            ...
            for index in range(len(state_list)//batch_size):
                ...
                batch_data = PretrainBatchData(state_list[left_index:right_index], label_list[left_index:right_index], self.device)
    ...
    def pretrain_train(self):
        ...
        with open("/".join([self.pretrain_log_path, pretrain_log_name]), "a") as f:
            ...
            for _ in tqdm(range(self.pretrain_epoch_num)):
                print("epoch: ", _)
                f.write("epoch: "+str(_)+'\n')
